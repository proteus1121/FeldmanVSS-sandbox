...
public class Scheme {
   ...
   */
  public Map<Integer, BigInteger> split(byte[] secret) {
    ...
    final Map<Integer, BigInteger> parts = new HashMap<>(n());
    for (int i = 0; i < values.length; i++) {
      parts.put(i + 1, new BigInteger(values[i]));
    }
    return Collections.unmodifiableMap(parts);
  }

  public Pair<Map<Integer, BigInteger>, List<BigInteger>> splitByFeldman(byte[] secret) {
    // generate part values
    final byte[][] values = new byte[n][secret.length];
    final List<BigInteger> checkSum = new ArrayList<>();
    checkSum.add(new BigInteger("5"));
    checkSum.add(new BigInteger("2"));
    checkSum.add(new BigInteger("1"));

    for (int i = 0; i < secret.length; i++) {
      // for each byte, generate a random polynomial, p
//      final byte[] p = GF256.generate(random, k - 1, secret[i]);
//      checkSum.add(new BigInteger(p));
      for (int x = 1; x <= n; x++) {
        final byte[] p = checkSum.get(n).toByteArray();
        // each part's byte is p(partId)
        values[x - 1][i] = GF256.eval(p, (byte) x);
      }
    }
